...
const { quota } = require("/etc/drumee/conf.d/myDrumee.json");
...
class __private_media extends Media {
  ...
  async chk_pre_transact(src, dest) {
    ...
    if (this.heap.action == "copy") {
      ...
      if (
        disk_limit.watermark == Infinity ||
        (quota && quota.watermark == Infinity)
      ) {
        this._done();
      }
      ...
      if (avaialbe_disk_limit <= 0) {
        if (this.uid != disk_limit.owner_id) {
          error = Cache.message("limit_exceeded");
        }
        return this.exception.user(error);
      }
      let trans_size = await this.yp.await_proc(
        "get_transation_size",
        src,
        hub_id,
        this.heap.action
      );
      if (avaialbe_disk_limit < trans_size.size) {
        if (this.uid != disk_limit.owner_id) {
          error = Cache.message("limit_exceeded");
        }
        return this.exception.user(error);
   ...
   */
  home() {
    this.db.call_proc("mfs_home", this.output.data);
  }
  ...
  async save() {
    ...
    let filepath = resolve(process.env.DRUMEE_TMP_DIR, `${filename}`);
  ...
  async replace_content(node, incoming_file, filename, hash) {
    ...
    this.output.data({ ...node, ...attr, privilege, replace: 1 });
  }
