...
const { data_dir, tmp_dir, server_location, mfs_dir } = sysEnv();
 ...
 */
  ...
  async chekcDiskLimit(rid) {
    ...
    let avaialbe_disk_limit = curr_filesize + 1000;
    let disk_limit = await this.yp.await_proc("disk_limit", rid);
    let { watermark } = disk_limit;
    if (watermark == Infinity) return true;
    avaialbe_disk_limit = disk_limit.available_disk;
    if (curr_filesize > avaialbe_disk_limit) {
 ...
 */
  ...
  async download(id, vcf) {
    ...
    this.debug("AAA:1273", cmd, `'${str_args}'`)
    this.output.data({
      wait: 1,
      size,
      total_size,
  ...
  async zip() {
    ...
    this.debug("AAA:1321", { src, target })
